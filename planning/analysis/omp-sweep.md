...
- **Baseline**: `baseline-sweep_v1` = 1.4432e+06 GFLOP/s (PASSED). The immediate
  pre-OMP shipping config is `np-sweep` `2x4_row_491k` = 2.2067e+06.
...
| Threads | GFLOP/s | vs baseline (1.4432e6) | node |
|---|---:|---:|---|
| 1  | 1.1606e+06 | −19.58% | g16 |
| 2  | 1.7369e+06 | +20.35% | g17 |
| 4  | 2.1323e+06 | +47.75% | g18 |
| 8  | 2.2859e+06 | +58.39% | g19 |
| 10 | 2.2455e+06 | +55.59% | g19 |
| 12 | 2.2529e+06 | +56.11% | g16 |
| 14 | 2.2754e+06 | +57.67% | g17 |
| 16 | 2.2840e+06 | +58.26% | g16 |
| 18 | 2.2671e+06 | +57.09% | g17 |
| 20 | 2.2624e+06 | +56.76% | g18 |

The thread count is a real lever with a sharp low-end collapse: 1 thread
(−19.6%) and 2 threads (+20.3%) starve the host threads that drive the GPU
pipeline, and the sweep climbs steeply to a peak at **8 threads**. Above 8 the
curve does not climb further: 10–20 sit in a noise-dominated plateau between
2.2455e+06 and 2.2840e+06. Two consecutive declines at t18 (2.2671e+06) and
t20 (2.2624e+06) triggered the stop rule. **t8 = 2.2859e+06 is the phase-1
peak**, with t16 = 2.2840e+06 an essentially tied second (0.08% apart, well
inside the observed node noise of ±~1%).

### 3.2 Phase 2 — `OMP_PLACES × OMP_PROC_BIND` at 8 threads

| PLACES | BIND | GFLOP/s | vs baseline (1.4432e6) | node |
|---|---|---:|---:|---|
| cores   | TRUE   | 9.1505e+05 | −36.60% | g16 |
| cores   | FALSE  | 2.3032e+06 | +59.59% | g18 |
| cores   | CLOSE  | 9.0139e+05 | −37.55% | g17 |
| cores   | SPREAD | 8.1662e+05 | −43.42% | g18 |
| sockets | TRUE   | 2.3204e+06 | +60.79% | g17 |
| sockets | FALSE  | 2.3026e+06 | +59.55% | g19 |
| sockets | CLOSE  | 2.2987e+06 | +59.28% | g19 |
| sockets | SPREAD | 2.3152e+06 | +60.43% | g16 |

Two clean groups emerge:

- **`cores` + a binding mode (TRUE/CLOSE/SPREAD) is catastrophic** — ~0.9e+06,
  a −37% to −43% collapse. With `--bind-to none`, every rank's 8 threads
  `OMP_PLACES=cores` + bind collapse onto the same low-numbered cores, so all 8
  ranks oversubscribe a handful of CPUs. `cores` + `FALSE` (no binding) avoids
  this and is healthy (2.3032e+06).
- **`sockets` is uniformly strong** (2.2987e+06 – 2.3204e+06) regardless of bind
  mode, and is the best family. `sockets` + `TRUE` = **2.3204e+06** is the
  highest recorded result; `sockets` + `SPREAD` (2.3152e+06) is second, and
  `sockets` + `FALSE`/`CLOSE` are ~2.30e+06. Binding to sockets distributes the
  8×8 = 64 host threads across both NUMA nodes instead of squeezing them, which
  is a consistent net win.
...
- **Confirmed win**: `OMP_NUM_THREADS = 8` (+3.6% over no-OMP) and
  `OMP_PLACES = sockets` (+~1.5% further at 8 threads). Together they reach
  2.3204e+06 (+60.8% over the original baseline).
